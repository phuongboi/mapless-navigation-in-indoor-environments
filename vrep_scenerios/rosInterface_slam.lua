...
function resetRobot_cb(msg)
    ...
    allModelObjects=sim.getObjectsInTree(robotHandle) -- get all objects in the model
    sim.setThreadAutomaticSwitch(false)
    for i=1,#allModelObjects,1 do
        sim.resetDynamicObject(allModelObjects[i]) -- reset all objects in the model
    end
    sim.setObjectPosition(robotHandle,-1,{0.53053, -0.2, 0.13879})
    sim.setObjectOrientation(robotHandle,-1,{0.0,0.0,-math.pi/2})
